...
export default function LoginForm() {
  ...
  const [userType, setUserType] = React.useState<'super_admin' | 'org_admin'>('org_admin'); // Track user type for OTP
  ...
  const onLoginSubmit = async (values: z.infer<typeof loginSchema>) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        console.log('🔄 Status 202 - Additional authentication required:', data);
        
        if (data.requires_otp) {
          console.log('🔐 OTP required for user type:', data.user_type);
          
          // Determine which OTP endpoint to use
          const otpEndpoint = data.user_type === 'super_admin' 
            ? 'super-admin/login' 
            : 'org-admin/login';
          
          // Send OTP request
          const otpResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api/v1'}/auth/${otpEndpoint}/`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
  ...
  const onOtpSubmit = async (values: z.infer<typeof otpSchema>) => {
    ...
    try {
      ...
      const verifyEndpoint = userType === 'super_admin' 
        ? 'super-admin/verify' 
        : 'org-admin/verify';
      
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api/v1'}/auth/${verifyEndpoint}/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        if (result.requires_password_change) {
          ...
          console.log('🎫 Temporary token received:', result.temporary_token ? 'Yes' : 'No');
          ...
          if (result.temporary_token) {
            ...
            setError('No temporary token received. Please try logging in again.');
            return;
          }
        }
        ...
        if (userType === 'super_admin') {
          ...
          console.log('OTP verification successful, navigating to super-admin dashboard');
          router.push('/super-admin');
        } else {
          // Organization admin
          localStorage.setItem('user', JSON.stringify(result.user));
          console.log('OTP verification successful, navigating to org-admin dashboard');
